...
//            CapchaControllerSync capchaController = new CapchaControllerSync();
...
//            try
//            {
//                string current_language = HelperSyncController.GetCurrentLanguageFacebook(chromeDriver);
//                List<string> lst_language_accept = new List<string> { "vi", "en" };
//                if (!HelperSyncController.CheckListContainsString(current_language, lst_language_accept))

//                {
//                    chromeDriver.GotoURL("https://en-gb.facebook.com/");
...
//                            if (HelperSyncController.CheckStringContainKeyword(chromeDriver.GetURL(), lstUrlCheck))
...
//                                    chromeDriver.DelayTime(1);
//                                    if (chromeDriver.CheckElements(4, "[role=\"dialog\"] input[value=\"1\"]", 0, 0, "", 0, 2, 0.1))
//                                    {
...
//                                    if (!string.IsNullOrEmpty(nodeChecking) || HelperSyncController.CheckStringContainKeyword(chromeDriver.GetURL(), lst_link_errsword))
//                                    {
...
//                            chromeDriver.ClearTextAndSendKeys(4, node + " [name*=\"pass\"]", account.Password ?? "", 0, true, true);
//                            chromeDriver.DelayTime(1);
...
//                                if (resultLogin.IsError()) return Result.Error<TAccount>(resultLogin.Message);
//                                chromeDriver.DelayTime(0.5);
//                                chromeDriver.ClearTextAndSendKeys(4, "[role=\"dialog\"] [name*=\"pass\"]", account.Password ?? "", 0, true, true);
//                                chromeDriver.DelayTime(1);
...
//                                {
//                                    chromeDriver.DelayTime(2);
//                                    chromeDriver.Click(4, "[aria-label=\"Continue\"]");
//                                    continue;
//                                }
//                                break;
//                            }
...
//                    if (HelperSyncController.CheckStringContainKeyword(chromeDriver.GetURL(), lst_url_verify_mail))
//                    {
//                        //verify by hotmail
//                        _lstDomain1secmail = AppInfo.SettingDetect.GetValue("domain_1sec_mail").Split('|').ToList();
//                        if (HelperSyncController.CheckStringContainKeyword(account.Email ?? "", _lstDomain1secmail))
//                        {
...
//                        if (!string.IsNullOrEmpty(account.Privatekey))
//                        {
//                            code_verify = _loginControllerl.Get2FAFacebook(account.Privatekey);
//                        }
//                    }
...
//                        if (HelperSyncController.CheckStringContainKeyword(currentUrl, lstUrl2FAUI))
//                        {
//                            lstNode = new List<string> { "//div[@role=\"dialog\"]//div[@role=\"button\"]" };
//                            node = chromeDriver.GetElementExistFromList(3, 0.5, lstNode);
//                            if (!string.IsNullOrEmpty(node))
...
//                        nodeChecking = chromeDriver.GetElementExistFromList(4, 0.5, lstNodeChecking);
//                        if (HelperSyncController.CheckStringContainKeyword(chromeDriver.GetURL(), lstUrlCheck) || string.IsNullOrEmpty(nodeChecking))
//                        {
...
//        {
//            Result resultLogin = new Result { Code = StatusLoginAccountFacebook.Error.ToString() };
...
//                if (IsLoginedAccount(chromeDriver))
//                {
//                    string uid_page_now = chromeDriver.ExecuteScript("return require('CurrentUserInitialData').USER_ID;");
...
//                if (HelperSyncController.CheckStringContainKeyword(chromedriver.GetURL(), lstUrlChecking))
//                {
//                    return true;
//                }

//                List<string> lstKeyHtml = new List<string> { "/friends/", "\"is_checkpointed\":false" };
//                if (HelperSyncController.CheckStringContainKeyword(chromedriver.GetPageSource(), lstKeyHtml))
//                {
//                    return true;
//                }
